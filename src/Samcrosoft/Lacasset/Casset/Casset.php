<?php
namespace Samcrosoft\Lacasset\Casset;
use Samcrosoft\Lacasset\Casset\Core as Core;
...
class Casset
{
	...
	*/
	...
	public function __construct()
	{
		...
		$paths = Config::get('lacasset::paths') ? Config::get('lacasset::paths') : self::$asset_paths;

		self::$symlinks = Config::get('lacasset::symlinks') ? Config::get('lacasset::symlinks') : array();

		foreach ($paths as $key => $path) {
			self::add_path($key, $path);
		}

//		self::$asset_url = Config::get('lacasset::url') ? Config::get('lacasset::url') : Config::get('app.url');
        // do more on this
        self::$asset_url = \Config::get('lacasset::url');
        if (!self::$asset_url)
            self::$asset_url = preg_replace('#^https?://#','//', \Request::root());
        self::$asset_url = rtrim(self::$asset_url, '/') . '/';

		self::$default_folders = array(
			'css' => Config::get('lacasset::css_dir'),
			'js' => Config::get('lacasset::js_dir'),
			'img' => Config::get('lacasset::img_dir'),
		);

		is_null(Config::get('lacasset::cache_path')) or self::$cache_path = Config::get('lacasset::cache_path');
		is_null(Config::get('lacasset::min')) or self::$default_options['min'] = Config::get('lacasset::min');
		is_null(Config::get('lacasset::combine')) or self::$default_options['combine'] = Config::get('lacasset::combine');
		is_null(Config::get('lacasset::deps_max_depth')) or self::$deps_max_depth = Config::get('lacasset::deps_max_depth');

		$group_sets = Config::get('lacasset::groups') ? Config::get('lacasset::groups') : array();
		...
		is_null(Config::get('lacasset::show_files')) or self::$show_files = Config::get('lacasset::show_files');
		is_null(Config::get('lacasset::show_files_inline')) or self::$show_files_inline = Config::get('lacasset::show_files_inline');
		is_null(Config::get('lacasset::post_load_callback')) or self::$post_load_callback = Config::get('lacasset::post_load_callback');
		is_null(Config::get('lacasset::filepath_callback')) or self::$filepath_callback = Config::get('lacasset::filepath_callback');
	}
	...
	public static function set_path($path_key = 'core')
	{
		if (!array_key_exists($path_key, self::$asset_paths)) {
			throw new AssetException("Asset path key $path_key doesn't exist");
		}
	...
	protected static function add_group_base($group_type, $group_name, $options = array())
	{
		...
		if (array_key_exists($group_name, self::$groups[$group_type])) {
			throw new AssetException("Group $group_name already exists: can't create it.");
		}
	...
	public static function set_group_option($type, $group_names, $option_key, $option_value)
	{
		...
		foreach ($group_names as $group_name) {
			...
			if (!self::group_exists($type, $group_name)) {
				throw new AssetException("Can't set option for group '$group_name' ($type), as it doesn't exist.");
			}
	...
	public static function render_js($group = false, $inline_dep = null, $attr_dep = array())
	{
		...
		foreach ($file_groups as $group_name => $file_group) {
			...
			if (self::$groups['js'][$group_name]['combine']) {
				...
				if ($inline) {
					$ret .= self::html_tag('script', $attr, PHP_EOL.file_get_contents(public_path()."/".self::$cache_path.$filename).PHP_EOL).PHP_EOL;
				} else {
	...
	public static function render_css($group = false, $inline_dep = null, $attr_dep = array())
	{
		...
		foreach ($file_groups as $group_name => $file_group) {
			...
			if (self::$groups['css'][$group_name]['combine']) {
				...
				if ($inline) {
					$ret .= self::html_tag('style', $attr, PHP_EOL.file_get_contents(public_path()."/".self::$cache_path.$filename).PHP_EOL).PHP_EOL;
				} else {
	...
	protected static function find_files($file, $asset_type)
	{
		...
		if (!array_key_exists($parts[0], self::$asset_paths)) {
			throw new AssetException("Could not find namespace {$parts[0]}");
		}
		...
		if ($remote) {
			...
			if (!$glob_files || !count($glob_files)) {
				throw new AssetException("Found no files matching $path$folder$file");
			}
	...
	protected static function resolve_deps($type, $group_names, $depth = 0)
	{
		if ($depth > self::$deps_max_depth) {
			throw new AssetException("Reached depth $depth trying to resolve dependencies. ".
	...
	protected static function combine($type, $file_group, $minify, $inline)
	{
		...
		foreach ($file_group as $file) {
			...
			$mod = filemtime(public_path()."/".$file['file']);
			if ($mod > $last_mod) {
				$last_mod = $mod;
			}
		}
		...
		$filepath = public_path()."/".self::$cache_path.'/'.$filename;
		...
		if ($needs_update) {
			...
			foreach ($file_group as $file) {
				...
				if ($file['minified'] || !$minify) {
					...
					if ($type == 'css') {
						$content .= Core\Cssurirewriter::rewrite($content_temp, dirname($file['file']), null, self::$symlinks);
					} else {
					...
					if ($file_content === false) {
						throw new AssetException("Couldn't not open file {$file['file']}");
					}

					if ($type == 'js') {
						$content .= Core\JSMin::minify($file_content).PHP_EOL;
					} elseif ($type == 'css') {
//						$css = Core\Csscompressor::process($file_content).PHP_EOL;
                        // find another way to compress the css
                        //@todo - use a library
                        $css = $file_content;
						$content .= Core\Cssurirewriter::rewrite($css, dirname($file['file']), null, self::$symlinks);
					}
				}
			}
	...
	protected static function clear_cache_base($filter = '*', $before = 'now')
	{
		...
		$files = glob(public_path()."/".self::$cache_path.$filter);
		foreach ($files as $file) {
